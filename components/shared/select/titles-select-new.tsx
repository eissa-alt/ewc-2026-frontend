...
type TitleSelectNewProps = {
   ...
   disabled?: boolean;
   placeholder?: string;
   titles?: TitleType[]; // Optional: if provided, skip API call
   category?: string; // Optional: category/role passed from parent component
};

const TitleSelectNew: React.FC<TitleSelectNewProps> = ({
   label,
   callBack,
   errors,
   selected_id,
   ...
   const [list, setList] = useState<TitleType[]>([]);
   ...
   useEffect(() => {
      // If titles are provided as props, use them and skip API call
      if (titles !== undefined) {
         setList(titles || []);
         setLoading(false);
         return;
      ...
      const fetchData = async () => {
         setLoading(true);
         try {
            const res = await Axios.get(`/titles-select/${role}?lang=${lang}`);
            if (!mounted) return;
   ...
   const options: Item[] = useMemo(() => {
      return (list || [])
         .filter(item => (gender ? item.allowed_genders?.includes(gender) : true))
         .map(item => {
            // EN: always item.name (already localized from API)
            // AR: prefer gender-specific if available, else fallback to item.name
            const labelText =
               lang === 'en'
                  ? item.name
                  : gender === 'male'
                  ? item.gender_specific_ar?.male || item.name
                  : gender === 'female'
                  ? item.gender_specific_ar?.female || item.name
                  : item.name;
   ...
   // If the selected id no longer exists under current gender filter, clear it
   ...
   const resolvedPlaceholder = placeholder || translate({ id: 'web:select_title' }) || 'Select...';
